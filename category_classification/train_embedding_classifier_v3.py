#!/usr/bin/env python3
"""
Train EmbeddingClassifierV3 with contrastive projection head.
"""
...
from category_classification.embedding_classifier_v3 import (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train EmbeddingClassifierV3 with contrastive loss.")
    p.add_argument("--data", type=str, default="./lspc_dataset_full", help="Path to HF DatasetDict.")
    p.add_argument("--out", type=str, default="./embedding_classifier_prodcat_v3", help="Output directory.")
    ...
    p.add_argument("--contrastive_dim", type=int, default=256, help="Projection dimension for contrastive head.")
    p.add_argument("--contrastive_weight", type=float, default=0.1, help="Weight for contrastive loss term.")
    p.add_argument("--contrastive_temp", type=float, default=0.05, help="Temperature for contrastive loss.")
    p.add_argument("--projection_dropout", type=float, default=0.1, help="Dropout inside projection head.")
...
def supervised_contrastive_loss(
    ...
    projections = F.normalize(projections, p=2, dim=-1, eps=1e-6)
    temperature = max(float(temperature), 1e-4)
    sim_matrix = projections @ projections.T / temperature
    sim_matrix = torch.clamp(sim_matrix, min=-20.0, max=20.0)
    mask = torch.eye(sim_matrix.size(0), device=projections.device, dtype=torch.bool)
    sim_matrix = sim_matrix.masked_fill(mask, float("-inf"))

    label_matrix = labels.unsqueeze(0) == labels.unsqueeze(1)
    if label_matrix.dtype != torch.bool:
        label_matrix = label_matrix.bool()
    ...
    mean_log_prob_pos = log_prob.sum(dim=1) / positive_counts.clamp(min=1.0)
...
def save_language_metrics(
    ...
    print(f"Saved validation language metrics -> {path.name}")
...
def main() -> None:
    ...
    if not LANG_DETECTOR:
        ...
        print("Language metrics explicitly disabled via CLI.")
    else:
        print(f"Language metrics enabled via {LANG_DETECTOR}.")

    config = EmbeddingClassifierV3Config(
    ...
    model = EmbeddingClassifierV3(config).to(device)
    ...
    if scheduler:
        print(
            f"Scheduler: {args.lr_schedule} | warmup={args.lr_warmup_steps} | "
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for step, batch in enumerate(loaders["train"], start=1):
            ...
            with autocast(device_type="cuda", enabled=use_amp):
                ...
                if args.contrastive_weight > 0 and "projection" in outputs:
                    contrastive_loss = supervised_contrastive_loss(
                        outputs["projection"],
                        labels,
                        args.contrastive_temp,
                    )
                    total_batch_loss = total_batch_loss + args.contrastive_weight * contrastive_loss
                loss = total_batch_loss / args.grad_accum
        ...
        if lang_metrics_enabled:
            ...
            lang_codes = ds["validation"]["language"] if "language" in ds["validation"].column_names else None
    ...
    if best_state:
        print(f"Reloading best validation macro-F1 weights ({best_macro_f1:.4f}).")
    ...
    if lang_metrics_enabled and test_labels is not None and test_preds is not None:
        print("Language metrics computing...")
        lang_texts = ds["test"]["text"]
        lang_codes = ds["test"]["language"] if "language" in ds["test"].column_names else None
        top_k = args.lang_top_k if args.lang_top_k > 0 else None
        language_metrics = compute_language_metrics(
            labels=test_labels,
            preds=test_preds,
        ...
        if not language_metrics:
            print("No languages met the minimum sample threshold.")
        else:
